...
// RESTAPIHandler provides a convenient interface for Category REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Category REST API handler.
...
// GetByID is the REST API handler for `GET /api/categories/{id}`.
...
// Get is the REST API handler for `GET /api/categories`.
...
// Create is the REST API handler for `POST /api/categories`.
...
// UpdateByID is the REST API handler for `PUT /api/categories/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/categories/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/categories/{id}`.
